...
namespace TapahtumaApi
{
    class Janne
    {
        ...
        public static async Task JannenMetodi()
        {
            ...
            ActivitiesList activity = await ApiHelper.RunAsync<ActivitiesList>(url, urlParams);


            var action = activity.data.Where(esko => esko.name.fi.Contains(haeLaji));
            ...
            foreach (var item in action)
            {
                ...
                Console.WriteLine("Kuvaus: " + item.description.body.Replace("<br>", "").Replace("<p>", "").Replace("<span>", "").Replace("<br />", "").Replace("</span>", "").Replace("</p>", "").Replace("&nbsp;", ""));
